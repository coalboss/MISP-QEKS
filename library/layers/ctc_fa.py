...
def make_pad_mask(seq_lens, max_time=None):
...
def _flip_label_probability(log_probs, xlens):
    ...
    rotate = (torch.arange(xmax, dtype=torch.int64)[:, None].to(xlens) + xlens) % xmax
...
def _flip_path_probability(cum_log_prob, xlens, path_lens):
    ...
    rotate_input = ((torch.arange(xmax, dtype=torch.int64)[:, None].to(xlens) + xlens) % xmax)
...
class CTCForcedAligner(object):
    ...
    def __call__(self, logits, elens, ys, add_eos, maxlen=None):
        ...
        with torch.no_grad():
            ...
            # zero padding
            mask = make_pad_mask(elens.to(logits.device), maxlen)
            ...
            trigger_points, best_aligns = self.align(log_probs, elens, ys_in_pad, ylens, add_eos=add_eos)
        return trigger_points, best_aligns
...
# ctc_fa=CTCForcedAligner(blank=0)
# B=2
# T=10
# H=3
# x=torch.randn(B,T,H)
# xl=torch.from_numpy(np.array([3,10]))
# y=[[1,2],[2,2,1]]
# tg, best=ctc_fa(x, xl, y, add_eos=False)
# print(best.size())
# print('x:',x)
# print('y:',y)
# print('tg:',tg)
