...
class QuantConv1d(nn.Conv1d):
    ...
    def __init__(self,
        ...
        output_clamp_val: float=32.0,
        weight_quant_bit: float=8.0,
        bias_quant_bit: float=16.0,
        input_quant_bit: float=8.0,
        output_quant_bit: float=16.0,
    ):
        ...
        super(QuantConv1d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode) # False, _single(0),
    ...
    def forward(self, input):
        ...
        if self.clamp:
            ...
            input = input.clamp(-self.input_clamp_val, self.input_clamp_val*(2.0**(self.input_quant_bit-1)-1.0)/(2.0**(self.input_quant_bit-1)))
        ...
        if self.padding_mode == 'circular':
            ...
            output = F.conv1d(F.pad(input, expanded_padding, mode='circular'), weight, bias, self.stride, _single(0), self.dilation, self.groups)
        else:
            output = F.conv1d(input, weight, bias, self.stride, self.padding, self.dilation, self.groups)
        
        if self.clamp:
            output = ClampFunction.apply(output, self.output_clamp_val, self.output_quant_bit)
        if self.quant:
            output = QuantFunction.apply(output, self.output_clamp_val, self.output_quant_bit)
        return output

class CasualConv2d(nn.Conv2d):
    '''
    ...
    def __init__(self,
        ...
        casual_dim: int = 2,
        right_context: int = 0):
        super(CasualConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
    ...
    def forward(self, x):
        ...
        out = self._conv_forward(x_left_pad, self.weight)
        return out
...
class QuantConv2dFreedom(nn.Conv2d):
    '''
        casual_dim=2, 针对数据(N,C,T,F)，T为时间维度
        casual_dim=3, 针对数据(N,C,F,T)，T为时间维度
    '''
    def __init__(
        ...
        super(QuantConv2dFreedom, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias, padding_mode)
...
class QuantConvTranspose2dFreedom(nn.ConvTranspose2d):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        super(QuantConvTranspose2dFreedom, self).__init__(in_channels, out_channels, kernel_size, stride, padding, output_padding, groups, bias, dilation, padding_mode)
